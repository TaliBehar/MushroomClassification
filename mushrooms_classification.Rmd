...
1. Detrmain which features are most indicative of a poisonous mushroom

2. Detrmain what types of machine learning models perform best on 'Mushroom Classification' data set by achiving model accuracy of 100% 
...
Random forest, Knn and Classification trees(model 1.2) achived the perfect 100% accuracy.

## Conclusion

* The chosen models for clasifing mushrooms class are Random Forest and Knn

* The most important featurs for the classification are 'spore.print.color', 'gill.color', 'stalk.surface.above.ring' and 'stalk.surface.below.ring' 
...
In order to avoid overfitting, we'll use 10-folds cross validation. The splits be done by 80%-20% training and test set 
```{r train ct model}
...
As we predicted before, 'Odor' is a dominant feature and is the first to be splited, The next feature to be splited is 'Spore print color', while the last is 'Stalk root'. 

Notice that the Class. trees algoritm splitted 4 features out of 21. 
...
We can interperate it as how valuable or neaded this variable to the split.
We can see that 8 out of the 21 featurs did not contribute any information to the splits , but "Stalk" char. had valuable influence overall.  
...
Yet, 'spore.print.color' and 'ring.color' have large contribution to both of the models. 
