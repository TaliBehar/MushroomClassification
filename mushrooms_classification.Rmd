...
Notice that the Desicion tree algoritm splitted 4 features out of 21. 
...
Classification tree model achived "only" 0.998 of accuracy, with false-positibe mistake of 3 edible observation that predicted as poisonuse. In true-life terms, this mistake is forgiven, but it wasn't if the opposite has occure. 
...
The visual model achived worse result than the original - 99.6% accuracy.  
...
The Random forests combines multiple decision trees. Its goal is to improve prediction performance
...
When Knn is used in classification with catrgorical data, the basic idea is that each category is mapped into a real number in some optimal way, and then knn classification is performed using those numeric values. 
...
Random forest, Knn and Classification trees(tuned - variable ommited) achived the perfect 100% accuracy.

The first two class. tree models achive high, but not enogh, accuracy while the latter got a fatal false-negative mistake by classifing poisonus as edible. 
