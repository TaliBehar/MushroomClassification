...
Each column contain its own factors, represented by unique letters (see extension on 
section... < TODO>)
...
2. Determine what types of ML models perform best on 'Mushroom Classification' data set by achieving model accuracy of 100%, in a reasonable runtime. 
 ...
 * Constract canonical dataset for applying the KNN algorithm.  
...
Random forest, KNN and Classification trees (model 1.2) achieved the perfect 100% accuracy, but the RF model was the fastest among them, by ~<TODO>%.

## Conclusion

* The chosen models for classifying mushrooms class are Random Forest and KNN.
...
Each row represent one observation on one mushroom and contain its class and unique characteristics.


Moving forward, we'll learn about the mushrooms characteristics and their influence on the mashroom's classification.
...
'veil.type' has only 1 level which is not contributing infomation as a feature. We'll remove it from the data and won't use it as a feature. 

Some observations contain '?' in <TBS>, and we chose to use it as a regular factor, as the distribution is equal between edible and poisonous, and if we omitted these rows, we would lose 40% of the data.
...
Predicting poisonous mushrooms as edible can be a terrible mistake, so in addition to the accuracy of the model, we'll also take a look of the other parts of the 'confusionMatrix', that contains the sensitivity, specificity, prevalence and balanced accuracy.    
...
```
The rows and the columns represent the 21 features. The diagonal represent the correlation of the feature with itself (always correlated).  
...
Among the common classification algorithms are clssification/desicion trees, Random forest, and KNN. The latter works slightly differnt than the first two. 
...
train_ct$bestTune
```
The complexity parameter (cp) is the minimum improvement in the model that is needed in each node. 

When cp=0 then the tree is fully grown and no need to prune it.  
...
Notice that the classification trees algoritm splitted 4 features out of 21. 
...
```{r figure 11 Graph variable importhance ct, echo=FALSE, out.width = '70%'}
varImp(fit_ct)%>%
  mutate(char = rownames(.))%>%
...
Classification trees model achived "only" 0.998 of accuracy, with false-positibe mistake of 3 edible observation that predicted as poisonuse. In true-life terms, this mistake is forgiven, but it wasn't if the opposite has occure. 

## Model 1.1 - Classification trees - only "Visual Characteristics" 

The previos model took into considaration the entire variables and we saw that 2 of the general char. played a significant role in the splitting and importance. Yet, we got accuracy of 99.8% and false-positive mistake of 3 observation. 

Let's look into a case which we are asking to clasify a mushroom by a picture. What are the best featurs for the classification would be? 

  * create new dataset, omit the general char. 
...
# Test_visual will be 20% of visual_mashrooms data
visual_test_index <- createDataPartition(y = visual_mashrooms$class, 
                                         times = 1, p = 0.2, list = FALSE)
...
We'll repeat the previons steps and train the model to find the best complexity parameter with 10-fold cross-validation to avoid overfitting. 
...
```
The cross validation relative error (X-val) reaching the minimal value for a tree of size 20, which correspond to a complexity parameter of 0. 
This tree prouned with 16 more branches than the originat tree. 
...
rpart.plot(x = fit_ct_visual, type =5, extra = 100, 
           ...
           fallen.leaves=TRUE, tweak = 1)
```
 * Variable importhance 

Stalk still have large influence on the splitting, while ring type has the highst mean decrease gini (used to be zero in the original tree), means that gill type (the first var. to split) and the ring type is highly correlated
```{r figure 14 variable importhance tuned ct tuned, echo=FALSE, out.width='70%'}
...
The visual model achived lower result than the original - 99.6% accuracy.  

There is 5 false- positive results (5 edible predicted poisonus) and one false-negative result (poisonus predicted as edible). 

Means, one of the general char. is essential for the classification process. 
...
# Model 1.2 - Classification trees - tuned
  ...
  * create new dataset, omit the general char, return spore.print.color
```{r tuned_mashrooms}
tuned_mashrooms <- 
  mushrooms %>% 
...
```
  * Plot the tree 
```{r tuned tree plot, echo=FALSE, out.width='75%'}
# figure #
rpart.plot(x = fit_ct_tuned, type =5, extra = 100, 
           box.palette = c("lightblue","orangered"), 
           fallen.leaves=TRUE, tweak = 1)
```
  * Graph variable importhance 
```{r tuned variable importhance, echo=FALSE, out.width='70%'}
# figure 
varImp(fit_ct_tuned)%>%
  mutate(char = rownames(.))%>%
  ggplot(aes(x = reorder(char,Overall),y = Overall))+
...
The model achived a perfect accuracy of 100%. 
It can be assumed that 'Odor' that was a dominant feature in the original model was "stand in the way" toward a perfect prediction. Smell can be very confusing sometimes. 

In addition, the model suggest that 'gill attachment', 'veil color', and 'veil type' does not contributing information to the model and therfore are not needed, alongside with 'odor', 'population' and 'habitat' 
...
They are highly interpretable and easy to visualize (if small enough). They can model human decision processes and don’t require use of dummy predictors for categorical variables.

  * Classification trees limitations

The approach via recursive partitioning can easily over-train and is therefore a bit harder to train than. In terms of accuracy, it is rarely the best performing method since
it is not very flexible and is highly unstable to changes in training data. That is Why we saw diffrences in the above models, when we ommited/add featurs. 


## Model 2 - Random Forest

The Random forests combines multiple Class. trees. Its goal is to improve prediction performance
and reduce instability by averaging multiple Class. trees.

  * Train rf model and find best parameters

Same here, we will use 10-fold cross validation and set a tune_grid for mtry (number of variables randomly sampled as candidates at each split). We can see in the next plot that the accuracy is higher when mtry reach to 4 randomly sampled variables.  
```{r train rf model}
...
``` 
The accuracy improve as we add more trees until about 20 trees where accuracy stabilize
```{r figure 16 Plot ntree, echo=FALSE}
plot(fit_rf)
```
Random forest calculate the variable importance over ensembles of randomized trees.

The Random Forest first important variable is 'Odor'. In addition, all general char. are participaing in the classification and have a predictive power. 

Down the list, the less important featurs 'gill attachment' and 'veil color'. Those 2 featurs also wasn't needed in the tuned ct model that got 100% accuracy. Means, we can omit them. 
```{r figure 17 plot variable importhance - rf, echo=FALSE, out.width='70%'}
varImp(fit_rf)%>%
  mutate(char = rownames(.))%>%
  ggplot(aes(x = reorder(char,Overall),y = Overall))+
  geom_bar(stat = "identity", fill = "lightblue")+
...
```{r rf var importanc list, include=FALSE}
# var importance list 
rf_var_imp <- 
  varImp(fit_rf) %>%
  mutate(char = rownames(.)) %>% 
...
Random forest achived 100% accuracy.

Random forest chooses features randomly during the training process. Therefore, it does not depend highly on any specific set of features. It is more difficult to interpret than a single desicion tree, but the randomized feature selection makes random forest much more accurate than a decision tree.

This is why rf could handel 'Odor' better than the simple first model of class. tree. 
...
When Knn is used in classification with catrgorical data, the basic idea is that each category is mapped into a real number (dummy) in some optimal way, and then knn classification is performed using those numeric values. 
...
Since Knn cannot handle characters, we'll convert the mushrooms data set into data set with boolean variables, as the following overview. Then, we'll split the data into training and test sets. 
...
We'll use 10-folds cross-validation to avoid overfitting, and set a tune_grid for K in order to find the optimal parameter
```{r train knn }
...
train_knn$bestTune
```
The model achive the highest accuracy with k=3, means 3 variables in each neighborhood.

In case of low accuracy, we can suspact the lowst K as overtaining of the model
...
The low K=3 could lead us to an overtraining of the model, but even though and with the omission of two unneccery features, Knn achived 100% accuracy.  

  * limitation
  
Since Knn is based on the Euclidean distances, it unable to be applied directly on categorical data. In order to define the distance metrics for categorical variables, we need to preprocessing the dataset by transforming it to dummy variables that represent the categorical variables.

# The Models Results 

## The models accuracy

Random forest, Knn and Classification trees(tuned - variables ommited) achived the perfect 100% accuracy.

The first two class. tree models (1, 1.1) achive high, but not enogh, accuracy while the latter got a fatal false-negative mistake by classifing poisonus as edible. 
...
knitr::kable()
```
## The 15 most important featurs according to the 1.2 class. tree model and random forest

The next table represent the top 15 important variables (out of initial 21).

Each one of the following models consider different featurs as most important. 

Yet, 'spore.print.color' and 'gill.color' have large contribution to both of the models (those are highly correlated, since the gill is the part that produce the spore and it is only reasonable to share the color). 

In addition, 'gill.attachment' and 'veil.color' ommited from Knn and Class.trees model while plaing a very negligible, if any, role in random forest clasifier 
```{r var imp comparison, echo=FALSE}
print(data.frame("Class.trees"=ct_tuned_var_imp,"Random forest"=rf_var_imp))
```
...
Analysing and modeling the Mushrooms-classification data set lead us to a conclusion that models such as Knn and Random forest, which are more flexible and easy to train and tune, are better for classifying.

The limitation of each of the Knn and rf models that discused at the end of each section, did not hurt or biased the results. For class.trees, in my opinion, the model couldn't handle well with the task, since we needed some rough adjustment, including ommiting the most important var ('Odor') in order to achive 100% accuracy. Second, the model was very sensitive to changes and changed the size of the tree in a significant way.       

  * Which features are most indicative of a poisonous mushroom?

We can see that ommiting 'gill.attachment' and 'veil.color' from **all** of the models led us to 100% accuracy. 

The common 4 top important features from class.trees(1.2) model and rf model are 'spore.print.color', 'gill.color', 'stalk.surface.above.ring', 'stalk.surface.below.ring' and we must not ommit tham. 

  * Future work

As for the 'Odor', there is no certinty about its roll. 

I'd like to test some more classification algiritms such as 'Logistic Regression' and 'Support Vector Machines' to better ditrmain which algorithms perform best under its limitation and create a better stable list of featurs importance  
