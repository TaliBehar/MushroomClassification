...
```{r tuned variable importhance, echo=FALSE}
# figure 
varImp(fit_ct_tuned)%>%
  mutate(char = rownames(.))%>%
  ggplot(aes(x = reorder(char,Overall),y = Overall))+
...
Down the list, the less important featurs 'gill attachment' and 'veil color'. Those 2 featurs also wasn't needed in the tuned ct model that got 100% accuracy. Means, we can eliminate them. 
...
This is why rf could handel 'Odor' better than the simple first model of decision tree. 
...
## Model 3.1 - k-nearest neighbors - tuned 

```{r mushrooms_sanitized }
mushrooms_sanitized <- mushrooms %>% select (-gill.attachment, -veil.color, -veil.type) 
```

```{r convert sanitized and split, include=FALSE}
...
```{r Train fit predict knn tuned, include=FALSE}
# Train knn tuned model and find best parameters 
set.seed(201, sample.kind="Rounding") # if using R 3.5 or earlier, use `set.seed(201)`
train_control <- trainControl(method="cv", number = 5, p = 0.8)
tune_grid <- data.frame(k = seq(1, 15, by=1))
