...
The previous model took into consideration the entire variables and we saw that 2 of the general characteristics played a significant role in the splitting and importance. Yet, we got accuracy of 99.8% and false-positive mistake of 3 observation. 

Let's investigate a case which we are asking to classify a mushroom by an image. What are the best features for the classification would be? 
...
In addition, the model suggests that 'gill attachment', 'veil color', and 'veil type' does not contributing information to the model and therefore are not needed, alongside with 'odor', 'population' and 'habitat' 
...
The approach via recursive partitioning can easily over-train and is therefore a bit harder to train than. In terms of accuracy, it is rarely the best performing method since it is not very flexible and is highly unstable to changes in training data. That is Why we such big differences in the above models (particularly in the size of the trees), when we omitted/add features. 
...
Same here, we will use 10-folds cross validation and set a tune_grid for mtry (number of variables randomly sampled as candidates at each split). We can see in the next plot that the accuracy is higher when mtry reach to 4 randomly sampled variables.  
```{r train rf model}
...
Down the list, the less important features 'gill attachment' and 'veil color'. Those 2 features also wasn't needed in the tuned ct model that got 100% accuracy. Means, we can omit them. 
...
train_knn$bestTune
```
The model achieved the highest accuracy with k=3, means 3 variables in each neighborhood.
...
The low K=3 could lead us to an overtraining of the model, but even though and with the omission of two unnecessary features, KNN achieved 100% accuracy.  
  ...
  * limitation
  ...
  1. With 5-folds cross validation, the the overall runtime of the model was 97 seconds: 94 seconds for training, less than one second for fitting, and two seconds for predicting (10-folds cv increased the total runtime in 40 seconds and achieved the same model accuracy)

  2. Since KNN is based on the Euclidean distances, it unable to be applied directly on categorical data. In order to define the distance metrics for categorical variables, we need to preprocessing the dataset by transforming it to dummy variables that represent the categorical variables.
...
Analyzing and modeling the Mushrooms classification data set lead us to a conclusion that models such as KNN and Random forest, which are more flexible and easier to train and tune, are better for classifying.
...
I'd like to test some more classification algorithms such as 'Logistic Regression' and 'Support Vector Machines' to better determine which algorithms perform best under its limitation and create a better stable list of features importance.  
